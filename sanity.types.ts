...
// Query: *[_type == 'casestudy' && slug.current == $slug][0] {  _id,  _type,  _createdAt,  title,  'slug': slug.current,  excerpt,  "tableOfContents": content[style in ["h2"]],  content[],  relatedCaseStudies,  "relatedCasestudies": select(    relatedCaseStudies == "custom" => customRelatedPosts[]->{       _id,      _type,      _createdAt,      title,      'slug': slug.current,      excerpt,      image {         asset->{ url },         altText       }    },    relatedCaseStudies == "autofill" => *[_type == 'casestudy' && _id != ^._id][0...3]{       _id,      _type,      _createdAt,      title,      'slug': slug.current,      excerpt,      image {         asset->{ url },         altText       }    },  ),  "seo": {    "title": coalesce(seo.title, title, ""),    "description": coalesce(seo.description,  ""),    "noIndex": seo.noIndex == true,    "image": seo.image,  },}
export type CasestudyBySlugQueryResult = null;
// Variable: allCasestudiesQuery
// Query: *[_type == 'casestudy'] | order(orderRank asc) {  _id,  _type,  title,  'slug': slug.current,  excerpt,  image {     asset->{ url },     altText   },}
export type AllCasestudiesQueryResult = Array<never>;
// Variable: casestudiesExcludingFeaturedQuery
// Query: *[_type == 'casestudy' && _id != $featuredId] | order(orderRank asc) {  _id,  _type,  title,  'slug': slug.current,  excerpt,  image {     asset->{ url },     altText   },}
export type CasestudiesExcludingFeaturedQueryResult = Array<never>;
