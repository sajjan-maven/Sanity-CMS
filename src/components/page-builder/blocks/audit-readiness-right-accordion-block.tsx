...
export type AuditReadinessRightAccordionBlockProps =
    PageBuilderType<"auditReadinessRightAccordionBlock"> & {
        highlightedText?: string | null;
    };

export default function AuditReadinessRightAccordionBlock(
    props: AuditReadinessRightAccordionBlockProps
) {
    const { tagline, heading, highlightedText, description } = props;
    const accordionItems = props.accordionItems ?? [];
    ...
    // If no items, bail early (avoid rendering an empty accordion)
        ...
        <section className="pb-[60px] lg:pb-[100px] bg-[#f7f5f2]">
            <div className="max-w-[1256px] mx-auto px-4">
                ...
                <div className="flex flex-col items-center justify-center text-center">
                    ...
                    {heading && (
                        <h1 className="font-bold text-[28px] md:text-[36px] lg:text-[42px] leading-tight mt-4">
                            {highlightedText
                                ? heading.split(new RegExp(`(${highlightedText})`, "gi")).map((part, i) =>
                                    part.toLowerCase() === highlightedText.toLowerCase() ? (
                                        <span key={i} className="text-[#618d57]">{part}</span>
                                    ) : (
                                        part
                                    )
                                )
                    ...
                    {description && (
                        <p className="text-[#363338] mx-auto mt-4 text-base md:text-lg max-w-[560px]">
                            {description}
                        </p>
                    )}
                </div>
